...
def can_code_compile(
    ...
    code_file_name = "code.cpp" if not is_cuda else "code.cu"
...
class BlockParser:
    ...
    def visit_matcher(self):
        ...
        self.add_matcher(Matcher(self.data[:end], tags))
        self.consume_until(end)
    ...
    def diag(self, msg: str):
        print(f"{self.input_file}:{self.current_line + 1}: {msg}")
...
def parse_block(data: CommentedMatcher, input_file: Path) -> list[TestCase]:
    parser = BlockParser(data, input_file)
    parser.run()
    return parser.cases


def parse(data: list[CommentedMatcher], input_file: Path) -> list[TestCase]:
    return [
        case
        for cases in [parse_block(block, input_file) for block in data]
        for case in cases
    ]
...
def main():
    ...
    output_file_data = args.output_file.read_text().split("\n")

    res: str = (
        "\n\n".join(
            filter(
                lambda case: len(case) != 0,
                [r.build_test_case() for r in results],
            ),
        )
        + "\n"
    )
    ...
    for line in output_file_data:
        ...
        if re.match(r"\s*// doc_test_begin", line):
            keep_lines = False
            res = "\n".join(lines_to_keep) + "\n" + res
            lines_to_keep.clear()
            continue
        if re.match(r"\s*// doc_test_end", line):
            keep_lines = True
            lines_to_keep.append(line)

    res += "\n".join(lines_to_keep)

    args.output_file.write_text(res)
    print("Statistics:")
